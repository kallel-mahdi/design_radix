...
// Citable Color Palette System with Mood Variants
const palette = {
  // Shared foundation colors (same across all moods)
  foundation: {
    light: {
      background: {
      ...
      text: {
        ...
        tertiary: '#736B63',     // Placeholder, disabled states (WCAG AA: ~5.2:1)
    ...
    dark: {
      background: {
        ...
        secondary: '#0D0D0C',    // Barely lifted - sidebar, panels
        canvas: '#171614',       // Cards, elevated surfaces
        elevated: '#1E1D1A',     // Tooltips, dropdowns
      },
      text: {
        primary: '#E8E4DF',      // Warm off-white
        secondary: '#9A958D',    // Muted warm gray
        tertiary: '#857F77',     // Placeholder, disabled (WCAG AA: ~4.84:1)
  ...
  // Mood variants - only module colors change
  moods: {
    muted: {
      name: 'Muted',
      emoji: '😌',
      description: 'Most eye-friendly, ~30-35% saturation. Best for long reading sessions.',
      light: {
        modules: {
          bibliography: {
            primary: '#4A6A82',
    ...
    balanced: {
      name: 'Balanced',
      emoji: '🙂',
      description: 'Daily comfort, ~40-45% saturation. Good balance of energy and ease.',
      light: {
        modules: {
          bibliography: {
            primary: '#3A6D90',
          ...
          manuscripts: {
            primary: '#247A5E',   // WCAG AA: ~4.77:1 on cream bg
            light: '#E0F2EC',
            dark: '#1A624C',
          },
          discover: {
    ...
    vibrant: {
      name: 'Vibrant',
      emoji: '😊',
      description: 'Energetic, ~50-55% saturation. For shorter, focused sessions.',
      light: {
        modules: {
          bibliography: {
            primary: '#2E6E98',
...
// Helper to get combined palette for a specific mood and theme
function getPalette(mood, theme) {
  const foundation = palette.foundation[theme];
  const moodColors = palette.moods[mood][theme];
  
  return {
    name: theme === 'light' ? 'Citable Light' : 'Citable Dark',
    description: theme === 'light' 
      ? 'Warm, scholarly light mode inspired by aged paper and academic comfort'
      : 'True black OLED-optimized with warm undertones',
...
function ModuleCard({ module, mode, colors }) {
  return (
    <div style={{
...
function PDFMockup({ mode, colors }) {
  return (
    <div style={{
...
function SemanticColors({ mode, colors }) {
...
export default function CitablePalette() {
  ...
  const [mood, setMood] = useState('muted');
  const colors = getPalette(mood, mode);
  const moodInfo = palette.moods[mood];
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <div style={{
              ...
              {Object.entries(palette.moods).map(([key, m]) => (
                <button
                  key={key}
                  onClick={() => setMood(key)}
                  title={m.description}
                  style={{
                    ...
                    fontWeight: '600',
                    cursor: 'pointer',
                    backgroundColor: mood === key ? colors.modules.bibliography.primary : 'transparent',
                    color: mood === key ? '#FFFFFF' : colors.text.secondary,
        ...
        <div style={{
          ...
          margin: '12px auto 0',
          padding: '10px 14px',
          backgroundColor: colors.modules.bibliography.light,
          borderRadius: '8px',
          fontSize: '13px',
          color: colors.text.secondary,
        }}>
          <strong style={{ color: colors.text.primary }}>{moodInfo.emoji} {moodInfo.name}:</strong> {moodInfo.description}
        </div>
      </div>
      ...
      <div style={{
        ...
        <section style={{ marginBottom: '48px' }}>
          ...
          <p style={{
            ...
            Three distinct colors representing different cognitive modes. Current mood: <strong>{moodInfo.name}</strong> (~{mood === 'muted' ? '40%' : mood === 'balanced' ? '50%' : '60%'} saturation).
          </p>
          ...
          <div style={{
            ...
            {Object.values(colors.modules).map(module => (
              <ModuleCard key={module.name} module={module} mode={mode} colors={colors} />
            ))}
          </div>
        </section>
        ...
        <section style={{ marginBottom: '48px' }}>
          ...
          <SemanticColors mode={mode} colors={colors} />
        </section>
        ...
        <section style={{ marginBottom: '48px' }}>
          ...
          <PDFMockup mode={mode} colors={colors} />
        </section>
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{
                ...
                All three module colors share harmonized saturation ({mood === 'muted' ? '~40%' : mood === 'balanced' ? '~50%' : '~60%'} in {moodInfo.name} mode). 
        ...
        <section style={{ marginTop: '48px' }}>
          ...
          <div style={{
            ...
            <pre style={{ margin: 0 }}>
{`/* ${mode === 'dark' ? 'Dark' : 'Light'} Mode - Citable (${moodInfo.name}) */
